...
async function lookupViaTable(supabase, phone, nameSuffix) {
  ...
  for (const variant of phoneVariants(phone)) {
    const { data, error } = await supabase.from("bookings").select("booking_date, booking_time, status, service, location, full_name, phone").eq("phone", variant);
    if (error) throw error;
